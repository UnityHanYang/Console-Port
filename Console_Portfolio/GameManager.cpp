...
void GameManager::RandomEnemyUnit(int enemyLevel)
{
	...
	case 1:
		if (num == 0)
		{
			e_nj = new EnemyNinJa("(적)하급 겐지", "닌자", 1, 0, 0, 2, 70, 10, 10, 2, 6, 0);
			nd.PrintEnemyLowerNinJa();
			randomNum = num;
		}
		else if(num == 1)
		{
			e_ah = new EnemyArcher("(적)하급 한조", "궁수", 1, 0, 0, 2, 60, 10, 10, 2, 6, 0);
			bmd.PrintEnmeyBack();
	...
	case 2:
		if (num == 0)
		{
			e_inj = new IntermediateNInJa("(적)중급 겐지", "닌자", 7, 0, 0, 100, 100, 20, 20, 5, 10, 5);
			nd.PrintIntermediateNinJa();
			randomNum = num;
		}
		else if (num == 1)
		{
			e_iah = new IntermediateArcher("(적)중급 한조", "궁수", 7, 0, 0, 90, 90, 20, 20, 6, 13, 8);
			bmd.PrintEnmeyBack();
